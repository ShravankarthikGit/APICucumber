...
# Scenario with data driven testing using Scenario Outline and Examples table
@AddPlaceWithMultipleData	
...
@GetPlace
Scenario: Verify if place is being added using add place api and get placeID
	Given Add Place payload with "<name>" "<language>" "<address>"
	When user calls "AddPlaceAPI" with "POST" http request
	Then the response is success with status code 200
	And "status" in response body is "OK"
...
@DeletePlace	
Scenario: Verify Delete with deletePlaceAPI
